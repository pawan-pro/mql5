...
double totaldailyLoss = 0.0;
double totalLoss = 0.0;
input double Daily_Loss = 0.05;
input double Total_Loss = 0.1;
...
int OnInit(){
   ...
   maxDailyLoss = initialAccountBalance * Daily_Loss ; //5% of the initial account balance
   maxTotalLoss = initialAccountBalance * Total_Loss; //10% of the initial account balance
...
void OnTick(){
  //Reset daily loss if new day
    ...
    if(currentTime.day_of_year != lastDay.day_of_year){
         lastDay = currentTime;
         totaldailyLoss = totaldailyLoss + dailyLoss;
         dailyLoss = 0.0;
  ...
  dailyLoss = initialAccountBalance - currentEquity; //daily loss
  totalLoss = initialAccountBalance - currentEquity; //total loss

  if(totaldailyLoss > maxDailyLoss || totalLoss > maxTotalLoss){
      Print("Total Daily Loss: ", totaldailyLoss, " Total Loss: ", totalLoss);
